...
const projectSkillToolsMock = mock((_history: Message[], _opts: unknown) => ({
  allowedToolNames: new Set<string>(),
  toolDefinitions: [],
}));

mock.module("../daemon/conversation-skill-tools.js", () => ({
  projectSkillTools: projectSkillToolsMock,
}));

beforeEach(() => {
  projectSkillToolsMock.mockClear();
});
...
describe("createResolveToolsCallback — toolsDisabledDepth", () => {
  ...
  test("preactivates the Worklin retention skill by default", () => {
    const toolDefs = [makeToolDef("skill_execute")];
    const ctx = makeCtx();
    const resolve = createResolveToolsCallback(toolDefs, ctx)!;

    resolve(EMPTY_HISTORY);

    const options = projectSkillToolsMock.mock.calls.at(-1)?.[1] as
      | { preactivatedSkillIds?: string[] }
      | undefined;
    expect(options?.preactivatedSkillIds).toContain("worklin-retention");
  });
