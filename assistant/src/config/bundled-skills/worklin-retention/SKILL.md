...
For a real-data, review-only microsegment or campaign-idea request, call `retention_campaign_review_pilot` with the central brand ID. Start with `max_segments: 10`; resume with the returned `runId` when the ChatGPT subscription pauses or when the user explicitly asks to continue toward 50. This tool is the only bundled retention workflow allowed to generate these tranches. It is pinned to the personal `chatgpt-subscription` OpenAI OAuth connection and a compatible Codex model, never falls back to another provider, creates the editable **Customer Segments & Campaign Ideas** document only after the central run completes, and performs no Klaviyo mutation or send.
