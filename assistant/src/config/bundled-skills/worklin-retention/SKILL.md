...
For a real-data, review-only microsegment or weekly campaign request, treat a plain request such as "Create this week's campaigns" as enough instruction. First load `retention_brand_brain`. If the brand is new or incomplete, run the conversational onboarding below until the website, real product catalog, positioning, voice, brand rules, and public competitor research are ready. Do not draft generic campaigns while onboarding is incomplete.

Once the Brand Brain is ready, call `retention_campaign_review_pilot` with `max_segments: 10`. Omit `brand_id` when the workspace has one connected Klaviyo brand; the tool resolves it safely. The run freezes at most 500 currently consented profiles once and uses that same cohort for every audience definition and count. Resume with the returned `runId` when the ChatGPT subscription pauses or when the user explicitly asks to continue toward 50. This tool is the only bundled retention workflow allowed to generate these tranches. It is pinned to the personal `chatgpt-subscription` OpenAI OAuth connection and a compatible Codex model, never falls back to another provider, creates the editable **Customer Segments & Campaign Ideas** document only after the central run completes, and performs no Klaviyo mutation or send.

Keep the final chat response short. Point the user to the editable document and the review-only result card. If the run pauses, say that progress is saved and report the completed audience count; do not restart the run or switch providers.
