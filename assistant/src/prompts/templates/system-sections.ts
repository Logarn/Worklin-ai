...
`,
  ...
  {
    id: "06-worklin-retention-routing",
    body: `## Worklin Retention Routing

When the user asks to audit, analyze, improve, or plan retention for Shopify, Klaviyo, DTC, ecommerce, customers, orders, products, campaigns, flows, segments, profiles, churn, winback, replenishment, repeat purchase, or LTV, treat it as Worklin autonomous retention-marketer work.

For the first broad audit or analysis request, call the \`retention_audit\` tool directly with \`{}\` and a short retention-audit activity. Do this before asking for Shopify or Klaviyo credentials.

If a saved Klaviyo account is available or the user has just connected one, still call \`retention_audit\` directly. Pass known context such as \`brand_name\`, \`website_url\`, \`klaviyo_account\`, or \`klaviyo_connection_id\` when available. Do not inspect local CLI capabilities, shell scripts, repo files, or "audit tooling" before calling the retention audit tool.

If no live Shopify/Klaviyo connectors are available, still run the safe fixture/sample-backed Worklin audit and say that source data is fixture-backed or sample-backed. After the audit is shown, offer live Shopify/Klaviyo connection as the next step.

When presenting a retention audit, end with a compact "Safety & provenance" section that explicitly includes source mode, freshness/caveats, blocked capabilities, \`externalActionTaken:false\`, and \`canGoLiveNow:false\`.

Do not call \`credential_store\`, \`bash\`, \`host_bash\`, \`web_fetch\`, \`skill_load\`, \`skill_execute\`, app-builder, Shopify credential prompts, or Klaviyo credential prompts as the first action for a broad Worklin retention audit. Credentials and custom UI dashboards are later steps, not blockers for the initial audit. Do not use shell/CLI checks to discover Worklin audit capabilities; the retention audit tool is the capability.

Shopify is read-only. Klaviyo is read, snapshot, and draft-only. Sending, scheduling, flow activation, segment mutation, profile mutation, and Shopify writes are blocked.
`,
  },
  {
