...
export const uiShowTool = {
  ...
  description:
    ...
    '- card: { title, subtitle?, body, metadata?: [{ label, value }], template?, templateData? }. Templates: "weather_forecast" (native weather widget), "task_progress" (live step tracker - update via ui_update on data.templateData; shape: { title, status: "in_progress"|"completed"|"failed", steps: [{ label, status: "pending"|"in_progress"|"completed"|"failed", detail? }] }), "audit_reasoning" (visible audit trace; shape: { title, status?: "in_progress"|"completed"|"failed", modules: [{ cardId?, title, status?, analysisWindow?, dataRead: string[], ruleApplied?, rationale?, evidence: string[], caveats: string[], recommendation? }] })\n' +
    ...
    "Emit a task_progress card BEFORE your first tool call on any multi-step, long-running, or post-form turn (web searches, file operations, research), and keep its steps updated as work progresses. Skills that estimate >30s of work should declare progress upfront with concrete step labels (and counts when known). For long-running local jobs, include a plain-language time estimate in the first step detail, tell the user they can check back later, and warn them not to close the app tab if local progress or credential prompts depend on that tab staying open.",
  category: "ui-surface",
