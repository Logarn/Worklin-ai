...
| Concurrent multi-tenant runtime             | [`docs/concurrent-runtime-service.md`](docs/concurrent-runtime-service.md)                         |
...
- The concurrent runtime is a separate, opt-in chat tier with tenant-scoped conversation history and read-only web bootstrap APIs. It never changes process-global tenant identity: signed tenant context is explicit at service and repository boundaries, PostgreSQL uses compound ownership keys plus forced RLS, and unsupported routes fail closed to dedicated placement. Existing allocated assistants are never moved by enabling the placement flag; allowlisted failed or provisioning rows with no allocated service, volume, or gateway can recover onto the shared tier. See [`docs/concurrent-runtime-service.md`](docs/concurrent-runtime-service.md).
...
Work is the brand-first index for user-created output. The additive `artifacts` registry points to canonical Copybook, document, and app resources without copying their content. Registry rows carry a stable artifact ID, optional brand and parent IDs, canonical resource type/ID, artifact classification, metadata, favorite/archive state, and timestamps. Ambiguous legacy resources remain Unassigned; Copybook month documents stay nested beneath their annual Copybook rather than appearing as duplicate root cards.

The assistant exposes brand summaries and artifact list/detail/update routes under `/v1/assistants/:assistant_id`. Source registration is best-effort for ordinary documents and transactional for Copybooks. A missing source produces a recoverable registry item instead of blanking the entire collection. Reassignment, favorite, and archive operations update only the registry; canonical source identifiers and legacy deep links remain stable.
