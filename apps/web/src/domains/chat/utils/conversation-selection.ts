...
export function createDraftConversationId(): string {
  ...
  return `${DRAFT_CONVERSATION_PREFIX}${id}`;
}

export function isDraftConversationId(conversationId: string | null): boolean {
  return conversationId?.startsWith(DRAFT_CONVERSATION_PREFIX) ?? false;
}
