...
export function useConversationLoader({
  ...
  const defaultDraftConversationRef = useRef<{
    assistantId: string;
  ...
  useEffect(() => {
    ...
    // selected implicitly. When none exists yet, land on a client-side draft
    // key; the first send lets the assistant mint the real conversation id.
    ...
    let defaultConversationId = latestForeground?.conversationId;
    if (!defaultConversationId) {
      if (defaultDraftConversationRef.current?.assistantId !== assistantId) {
        defaultDraftConversationRef.current = {
          assistantId,
          conversationId: createDraftConversationId(),
        };
      }
      defaultConversationId =
        defaultDraftConversationRef.current.conversationId;
    }
