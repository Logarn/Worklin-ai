...
export function useOnboardingChoice({
  ...
  }, [phase, isNative, didOnboarding, messages, onboardingTasksEmpty, hasUserMessage, activeConversationId, onboardingConversationId]);
  ...
  // If the user sends a concrete task before touching the chooser, the
  // conversation has already answered the onboarding question implicitly.
  useEffect(() => {
    if (phase === "visible" && hasUserMessage) {
      setPhase("dismissed");
    }
  }, [phase, hasUserMessage]);
