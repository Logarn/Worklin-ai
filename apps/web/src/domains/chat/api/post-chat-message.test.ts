...
let mockLocalGatewayUrl: string | undefined;
let loadLockfileCalls = 0;
let primeLocalGatewayCalls = 0;
let mockLoadLockfile = async () => {
  loadLockfileCalls += 1;
};
mock.module("@/lib/local-mode", () => ({
  isLocalMode: () => mockIsLocalMode,
  getLocalGatewayUrl: () => mockLocalGatewayUrl,
  loadLockfile: () => mockLoadLockfile(),
  primeLocalGatewayConnectionWithRepair: async () => {
    primeLocalGatewayCalls += 1;
  },
}));
...
describe("postChatMessage onboarding payload", () => {
  ...
  beforeEach(() => {
    ...
    mockLocalGatewayUrl = undefined;
    loadLockfileCalls = 0;
    primeLocalGatewayCalls = 0;
    mockLoadLockfile = async () => {
      loadLockfileCalls += 1;
    };
  ...
  test("primes local gateway auth before sending in local gateway mode", async () => {
    mockIsLocalMode = true;
    mockLocalGatewayUrl = "/assistant/__gateway/20101";

    const result = await postChatMessage("asst-1", "K", "hello");

    expect(result.ok).toBe(true);
    expect(loadLockfileCalls).toBe(0);
    expect(primeLocalGatewayCalls).toBe(1);
    expect(capturedRequests).toHaveLength(1);
    expect(getRequestBody().content).toBe("hello");
  });

  test("loads the lockfile before local gateway auth when gateway url is not hydrated", async () => {
    mockIsLocalMode = true;
    mockLocalGatewayUrl = undefined;
    mockLoadLockfile = async () => {
      loadLockfileCalls += 1;
    ...
    expect(result.ok).toBe(true);
    expect(loadLockfileCalls).toBe(1);
    expect(primeLocalGatewayCalls).toBe(1);
    expect(capturedRequests).toHaveLength(1);
    expect(getRequestBody().content).toBe("hello");
  });
