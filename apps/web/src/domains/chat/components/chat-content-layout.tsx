...
export function ChatContentLayout(props: ChatMainPanelProps) {
  ...
  const handleExportDocument = useCallback(async () => {
    const doc = useViewerStore.getState().openedDocumentState;
    if (!doc || !assistantId) return;
    const pdfWindow = openPdfPreparingWindow(`${doc.documentName} PDF Export`);
    try {
      ...
      await presentPdfBlob(blob, safePdfFilename(doc.documentName), pdfWindow);
    } catch {
      pdfWindow?.close();
    }
  }, [assistantId]);
