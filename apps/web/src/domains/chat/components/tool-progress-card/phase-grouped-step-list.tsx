...
/**
 * Wrap a string in Typography while allowing long command/details text to
 * flow onto multiple lines inside the expanded progress card.
 */
function PillText({ children }: { children: ReactNode }) {
  return (
    <Typography
      variant="body-small-default"
      className="min-w-0 whitespace-normal break-words text-inherit leading-[18px]"
    >
      {children}
    </Typography>
  );
...
function StepPill({
  ...
  return (
    <div
      ...
      className={`inline-flex min-w-0 max-w-full items-start gap-1 self-start rounded-lg border px-2 py-1.5 ${toneClasses}`}
    >
      {children}
    </div>
  );
