...
type TranscriptContentGroup =
  ReturnType<typeof groupContentBlocks>[number];

function surfaceTemplate(surface: ConversationMessageSurface): string | undefined {
  const data = surface.data as { template?: unknown } | undefined;
  return typeof data?.template === "string" ? data.template : undefined;
}

function worklinAuditSurfaceRank(surface: ConversationMessageSurface): number {
  if (surface.surfaceType === "retention_audit") return 0;
  if (
    surface.surfaceType === "work_result" &&
    surface.surfaceId?.startsWith("audit-result-")
  ) {
    return 1;
  }
  if (
    surface.surfaceId?.startsWith("audit-reasoning-") ||
    surfaceTemplate(surface) === "audit_reasoning"
  ) {
    return 2;
  }
  return 3;
...
function orderWorklinAuditSurfaceRuns(
  groups: TranscriptContentGroup[],
): TranscriptContentGroup[] {
  const ordered: TranscriptContentGroup[] = [];
  let surfaceRun: TranscriptContentGroup[] = [];

  const flushSurfaceRun = () => {
    if (surfaceRun.length === 0) return;
    const hasAuditSurface = surfaceRun.some(
      (group) =>
        group.type === "surface" && worklinAuditSurfaceRank(group.surface) < 3,
    );
    ordered.push(
      ...(hasAuditSurface
        ? [...surfaceRun].sort((a, b) => {
            ...
            return (
              worklinAuditSurfaceRank(a.surface) -
              worklinAuditSurfaceRank(b.surface)
            );
          })
        : surfaceRun),
    );
...
function isLongWorklinAuditMarkdown(text: string): boolean {
  if (text.length < 2500) return false;
  const normalized = text.toLowerCase();
  const hasAuditTitle =
    normalized.includes("deep retention audit") ||
    normalized.includes("klaviyo l365 account audit") ||
    normalized.includes("klaviyo inventory audit");
  const hasAuditSections =
    normalized.includes("## executive summary") ||
    normalized.includes("## data trust") ||
    normalized.includes("## visual artifact pack");
  return hasAuditTitle && hasAuditSections;
}

function CollapsedWorklinAuditMarkdown() {
  return (
    <div className="w-full rounded-lg border border-[var(--border-base)] bg-[var(--surface-active)] px-4 py-3">
      <p className="text-label-small-default uppercase text-[var(--content-tertiary)]">
        Worklin audit report
      </p>
      <p className="mt-1 text-body-medium-default font-medium text-[var(--content-strong)]">
        The full audit is ready in the audit card.
      </p>
      <p className="mt-1 text-body-small-default text-[var(--content-secondary)]">
        Use Download PDF, Open PDF, View full audit, or Open editable doc
        below. Worklin keeps the chat short so the important actions stay easy
        to find.
      </p>
    </div>
  );
...
export function TranscriptMessageBody({
  ...
  const displayGroups = isUser ? groups : orderWorklinAuditSurfaceRuns(groups);
  ...
  const renderTextWithInlineSurfaces = (text: string, key: string) => {
    if (!isUser && isLongWorklinAuditMarkdown(text)) {
      return <CollapsedWorklinAuditMarkdown key={key} />;
    }
  ...
  const lastGroupIndex = displayGroups.length - 1;
  ...
  return (
    ...
    >
      <div className={columnClass}>
          {displayGroups.map((group, gi) => renderGroupNode(group, gi))}
        {hasAttachments && (
          <MessageAttachments
            attachments={message.attachments ?? []}
            assistantId={assistantId}
