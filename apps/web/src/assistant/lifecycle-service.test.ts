...
describe("lifecycleService — bootstrap branches", () => {
  ...
  test("local gateway boot primes auth before gateway-auth short-circuit", async () => {
    isLocalModeMock.mockImplementation(() => true);
    getLocalGatewayUrlMock.mockImplementation(
      () => "/assistant/__gateway/20101",
    );
    isGatewayAuthModeMock.mockImplementation(
      () => primeLocalGatewayConnectionWithRepairMock.mock.calls.length > 0,
    );
