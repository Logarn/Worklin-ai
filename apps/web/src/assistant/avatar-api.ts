...
export async function fetchAssistantCharacterProfile(
  assistantId: string,
): Promise<AssistantCharacterProfile | null> {
  try {
    const { data, error, response } = await workspaceFileGet({
      path: { assistant_id: assistantId },
      query: { path: ASSISTANT_CHARACTER_PROFILE_PATH },
      throwOnError: false,
    });
    assertHasResponse(response, error, "Failed to fetch character profile");
    if (!response.ok || !data) {
      return null;
    }

    const parsed: unknown = JSON.parse(data.content);
    if (!isAssistantCharacterProfile(parsed)) {
      return null;
    }
    return parsed;
  } catch {
...
export async function saveAssistantCharacterProfile(
  assistantId: string,
  profile: AssistantCharacterProfile,
): Promise<boolean> {
  try {
    const { error, response } = await workspaceWritePost({
      path: { assistant_id: assistantId },
      body: {
        path: ASSISTANT_CHARACTER_PROFILE_PATH,
        content: JSON.stringify(profile, null, 2),
      },
      throwOnError: false,
    });
    assertHasResponse(response, error, "Failed to save character profile");
    return response.ok;
  } catch {
    return false;
  }
