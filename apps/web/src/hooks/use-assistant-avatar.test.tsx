...
const fetchAssistantCharacterProfile = mock(
  async () => null as AssistantCharacterProfile | null,
);
...
afterEach(() => {
  ...
  fetchAssistantCharacterProfile.mockClear();
  ...
  fetchAssistantCharacterProfile.mockResolvedValue(null);
});
...
describe("useAssistantAvatar", () => {
  test("character kind exposes manifest traits and skips the image fetch", async () => {
    ...
    expect(result.current.characterProfile).toBeNull();
    expect(fetchCharacterComponents).toHaveBeenCalledTimes(1);
    expect(fetchAssistantCharacterProfile).toHaveBeenCalledTimes(1);
  ...
  test("none kind falls back with neither traits nor image", async () => {
    ...
    // both null ⇒ ChatAvatar falls back to default components / "W".
    expect(result.current.traits).toBeNull();
    expect(result.current.customImageUrl).toBeNull();
    expect(result.current.characterProfile).toBeNull();
    expect(fetchCharacterComponents).toHaveBeenCalledTimes(1);
  ...
  test("returns a saved TV-character profile with the existing avatar data", async () => {
    ...
    fetchAssistantCharacterProfile.mockResolvedValueOnce(profile);
    fetchAvatarState.mockResolvedValueOnce(characterState);
    ...
    await waitFor(() => {
      expect(result.current.characterProfile).toEqual(profile);
    });
  ...
  test("null state (transport failure) preserves the cached avatar instead of blanking", async () => {
    ...
    // Cached character avatar is preserved; it did not fall back to "W".
  ...
  test("null components preserves cached avatar on refetch instead of blanking", async () => {
    ...
    // THEN the cached character avatar is preserved (not blanked to "W")
    expect(second.result.current.traits).toEqual(traits);
