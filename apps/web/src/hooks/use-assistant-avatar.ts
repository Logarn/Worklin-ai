...
async function fetchAvatarViaManifest(
  ...
  if (state === null) {
    ...
    // showing the last good avatar instead of blanking out to the "W".
...
export function useAssistantAvatar(assistantId: string | null) {
  ...
  const { data, isLoading } = useQuery<AvatarData>({
    ...
    queryFn: async () => {
      ...
      const [components, characterProfile, { traits, imageUrl }] = await Promise.all([
        fetchCharacterComponents(id),
        fetchAssistantCharacterProfile(id),
        supportsManifest
    ...
    enabled: Boolean(assistantId) && assistantVersion !== null,
    staleTime: Infinity,
