...
function onboardingEntrypoint(isLocalMode: boolean): string {
  return isLocalMode ? routes.welcome : routes.onboarding.prechat;
}
...
//   5. Setup boundary     — incomplete users stay in one-time onboarding
//   6. Assistant required — completed users need at least one assistant
  ...
  if (
    ...
    return {
      ...
      to: hasCompletedOnboarding(state)
        ? routes.assistant
        : onboardingEntrypoint(state.isLocalMode),
    };
  }

  if (isOnboardingPath(path)) {
    const isPreview = pathnameWithSearch.includes("preview=true");
    if (
      path === routes.onboarding.prechat &&
