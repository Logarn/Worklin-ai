...
export function isPlatformDisabled(): boolean {
  ...
  return isLocalMode() && getPlatformAssistants().length === 0;
}
...
export async function saveLockfileAssistant(
  assistant: { assistantId: string; name?: string; cloud: string; runtimeUrl: string; hatchedAt: string; organizationId?: string; runtimeProvider?: string },
): Promise<void> {
...
export async function syncPlatformAssistantsToLockfile(
  assistants: Array<{ id: string; name?: string; is_local: boolean; created: string; runtime_provider?: string | null }>,
  ...
  const platformAssistants = assistants
    ...
    .map((a) => ({
      ...
      hatchedAt: a.created,
      ...(a.runtime_provider != null && { runtimeProvider: a.runtime_provider }),
...
export async function primeLocalGatewayConnection(
  ...
  const guardianToken = assistant
    ...
    : undefined;
  // A local gateway restart rotates the signing key, which makes an otherwise
  // unexpired cached actor token fail runtime-proxy validation with
  // `invalid_signature`. Priming is the recovery point, so mint a fresh token.
  clearGatewayToken();
