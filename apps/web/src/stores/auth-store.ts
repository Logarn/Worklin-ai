...
const useAuthStoreBase = create<AuthStore>()((set, get) => ({
  ...
  initSession: async () => {
    ...
    if (isLocalMode() && !isGatewayAuthEnabled()) {
      ...
      if (!suppressPlatformProbe) {
        probePlatformSessionIfReachable(set, { setUserOnSuccess: true });
      } else {
        set({ platformSession: "absent" });
      }
   ...
   * mis-seeded assistant via `wake` before surfacing any error. Boot uses the
   * same repairing path so a restarted gateway or stale guardian lease does not
   * strand the chat surface behind local 401s.
   */
