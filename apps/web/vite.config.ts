...
function isVercelBuild(env: Record<string, string | undefined>): boolean {
...
export default defineConfig(async ({ mode }) => {
  ...
  if (!isPlatformMode(env.VITE_PLATFORM_MODE)) {
    const { localModePlugin } = await import("./vite-plugin-local-mode");
    plugins.push(localModePlugin(env));
  }
  ...
  return {
    base: isVercelBuild(env) ? "/" : "/assistant/",
    plugins,
    resolve: {
      alias: [
        {
